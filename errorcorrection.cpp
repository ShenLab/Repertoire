...
string finalread="remove", finalqual="######"; // read and quality of completely merged reads (overlap and nonoverlap combined). Default is N,# to indicate that a suitable output read could not be produced from the two input reads.
...
if(rz1==2 && rz2==2){ // one site mapped for both reads. merge where possible to get longer read that can map in two places.
	...
	if(flag2==4){
        ...
        else if (chr1a==chr1b and chr1a!=chr2){
                finalread=seq1;
                finalqual=qual1;
                errorcase="2/0";
        }
...
[i]); overlapqual.push_back(qualuse[i]);}} // where neither matches reference use sequence 1, unless sequence 1 is N
		...
		if(chr1a==chr1b and chr2a!=chr2b and chr1a!=chr2a and chr1a!=chr2b) // treat as 2/0 case
                        {
                        errorcase="2/0";
                        finalread=seq1;
                        finalqual=qual1;
			/////////
			output<<"@"<<head<<"\t"<<errorcase<<endl<<finalread<<endl<<"+"<<endl<<finalqual<<endl; continue;
                        }
                else if (chr2a==chr2b and chr1a!=chr1b and chr2a!=chr1a and chr2a!=chr1b)   // treat as 2/0 case
                        {
                        errorcase="2/0";
                        finalread=seq2;
                        finalqual=qual2;
		...
		else if(chr1a==chr1b and chr1a!=chr2a and chr1a==chr2b) // treat as 3-2 case, and fix read 1 using part of read 2	
...
if(finalread.length()>0)
{
        ...
        {
		//cout<<finalread[i]<<endl;
                if (isprint(finalread[i]))
                {
                        tmp.push_back(finalread[i]);
                }
		else{
...
if(finalread.length()>0)
{
        for (int i=0; i<finalqual.length();i++)
        {
                if (isprint(finalread[i]))
                {
                        tmp2.push_back(finalqual[i]);
                }
        }
        finalqual=tmp2;
}
...
for (int i=0; i<finalread.length();i++)
		...
		{
			finalread="remove";
			finalqual="gappedalignment";	
		}
	}


if(finalread.length()==finalqual.length()){
	output<<"@"<<head<<"\t"<<errorcase<<endl<<finalread<<endl<<"+"<<endl<<finalqual<<endl;
        }
else if(finalread.length()!=finalqual.length()){
        output<<"@"<<head<<"\t"<<"2/1dump"<<endl<<"remove"<<endl<<"+"<<endl<<"######"<<endl;
	//cout<<rf<<endl<<rq<<endl;
	//output<<"@"<<head<<"\t"<<"gapped"<<endl<<finalread<<endl<<"+"<<endl<<finalqual<<endl;
	//exit(0);
	}
