...
open(FinIgHpos,'cassetteref/TRA.v37.ref.txt');
...
#print "$i\t$values[13]\n";
	...
	if ($#values > 3 && ( $values[9] !~ /N/ || $values[9]=~/^(N+)([^N]*)(N+)$/ || $values[9]=~/^(N+)([^N]*)$/ || $values[9]=~/^([^N]*)(N+)$/) ) {
	#if ($#values > 3 && $values[9] !~ /N/) 
		$startT[0] = $values[3];
		$endT[0] = $startT[0];	
					...
					if (($startTM[$i]>=$IgHanno[$j][2]&&$startTM[$i]<=$IgHanno[$j][3])||($endTM[$i]>=$IgHanno[$j][2]&&$endTM[$i]<=$IgHanno[$j][3])){
						...
						if (substr($IgHanno[$j][1],0,4) eq "TRAV"){  #$IgHanno[][7] is the DNA sequence for the gene
							...
							$readSeq = substr($values[9], $startRM[0]-1,$endRM[$#endRM]-$startRM[0]+1);
								#print $readSeq,"\n";
								#print $values[9],"\n";
								#print length($readSeq),"\n";
								#print length($values[9]),"\n";
								#print $startRM[0],"\n";
								#print $endRM[$#endRM],"\n";
								$postJseq=substr($values[9],$endRM[$#endRM],length($values[9])-$endRM[$#endRM]);
								#print $backseq,"\n";
							if ($tmpSpos<0){
								$VdJdI[1] = 0;
							}
